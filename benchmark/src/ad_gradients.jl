# Gradient benchmarks for LoweredDistributions across AD backends.
#
# Scenarios and the backend list are sourced from the `ADFixtures` path
# package at `test/ADFixtures`, which also drives the test suite
# (`test/ad/runtests.jl`). This keeps the two AD surfaces in lock-step.
#
...
# known-broken combinations (see the extensive commentary in
# `test/ADFixtures/src/ADFixtures.jl`, e.g. the CTMC builder on Enzyme) are
# silently omitted and the AirspeedVelocity suite can run unattended.
