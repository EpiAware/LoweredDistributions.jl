...
indexes by species and transition name, not by position). It starts from the
same `Gamma(3, 1.5)` lowering as the other backends — see
[Lowering a distribution to a dynamical system](@ref lowering-backends).

This backend's demo runs in its own environment: the package keeps one at
`test/algebraic_petri`, and the demo below runs against it.
